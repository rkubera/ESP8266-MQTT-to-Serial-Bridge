...
void WiFiManager::mydelay(int mymillis) {
  unsigned long mytimer = millis();
  while (abs(millis()-mytimer)<mymillis) {
...
bool WiFiManager::setupHostname(bool restart){
  ...
  bool res = true;
    ...
    mydelay(200); // do not remove, give time for STA_START
  ...
  if(restart && (WiFi.status() == WL_CONNECTED)){
    ...
    WiFi_Disconnect();
    mydelay(200); // do not remove, need a delay for disconnect to change status()
  }
...
bool WiFiManager::startAP(){
  ...
  mydelay(500); // slight delay to make sure we get an AP IP
...
uint8_t WiFiManager::processConfigPortal(){
    ...
    if(connect) {
      ...
      if(_enableCaptivePortal) mydelay(_cpclosedelay); // keeps the captiveportal from closing to fast.
...
/**
 * setPortalLoopCallback, set a portal callback if the portal is running
 * @access public
 * @param {[type]} void (*func)(void)
 */
